import { useApp, setToStore, getFromStore, useBlocks, pushByIndex, itemById } from "store";
import { getNestedChildren, prepareBlocksToClone, res } from "helpers";
export const keysHandler = (e, blocks, targeter, res) => { 
    if(document.activeElement.tagName !== "INPUT" && document.activeElement.tagName !== "TEXTAREA"){
      const key = e.which || e.keyCode, ctrl = e.ctrlKey ? e.ctrlKey : ((key === 17)
        ? true : false);
      if (key == 86 && ctrl) { // CTRL + V
        if(targeter.id){
          const toPaste = prepareBlocksToClone(getFromStore({store:'custom',ref:`blocksCopy`}))
          if(toPaste){
            toPaste?.[0].parentId = targeter.id;
            const item = itemById({store:'display',ref:'blocks.id', data:targeter.id})
            const index = item.index;
            toPaste.map((el,i) => { 
              pushByIndex({store:"display",ref:`blocks.${index+i+1}`, data:{...el,post:targeter.post, insert:true}})
            })
          }
